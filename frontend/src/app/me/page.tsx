...
function BoostPanel({ myId }: { myId: string }) {
  ...
  return (
    <section className="rounded-3xl border border-amber-300 bg-gradient-to-b from-amber-50 to-white p-5">
      ...
      <p className="mt-2 text-[11px] text-slate-500">{te ? "💳 Payment తర్వాత boost automatic ON (UPI: manavivaha@upi) — UTR ని /pricing లో ఇవ్వండి." : "💳 Boost auto-ON after payment (UPI: manavivaha@upi) — give UTR in /pricing."}</p>
      <Msg m={msg} />
    </section>
  );
}
